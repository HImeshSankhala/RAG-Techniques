...
At `k = 3` `cassandra.md#0` overtakes the correct chunk — one step earlier than the
inequality predicts, because `k > 3` is *strict* and `k = 3` is an exact tie at `1/4` each.
RRF defines no tie-break, so what actually settles the order there is `sorted()` being
stable and the dense list being merged first: an implementation detail of the merge, not a
property of the algorithm. Nothing is lost either way, because the chunk is still in the
window. It only falls out at `k = 7`, when the fourth-place competitor catches it:
`cassandra.md#3`, ranked **dense #9 and BM25 #8**, scoring `1/(k+9) + 1/(k+8)`. The pairwise
order flips at `k = 3`; eviction waits until `k = 7`, four steps further out — because RRF's
consensus bonus accrues to every chunk both retrievers saw, not only to the one at the top.
