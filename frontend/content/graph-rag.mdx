...
Graph RAG retrieves `kafka.md#4, bigtable.md#0, gfs.md#1, kafka.md#0`. `raft.md#1` — the
chunk the whole example exists to reach — is **never in the top 4**.

The reason is not a missed walk. Instrumented at two and three hops, with and without the
extraction's hallucinated edges, every target chunk is *inside* the traversal every time
and never ranked into the top 4. The ball reached from `Kafka` is 53 nodes touching **35 of
43 chunks — 81% of the corpus.**

So the graph is not acting as a filter. Once the walk returns four-fifths of everything,
selection *within* it does all the work — and a graph has no similarity signal to select
with. Four ranking rules were measured (depth-sorted, round-robin, edge-depth-weighted,
seed-coverage); the best one ships and still loses.
...
relative to the corpus — thousands of documents where two hops reach dozens, not 81%. The
technique is not wrong; this corpus is too small for it to be right.
